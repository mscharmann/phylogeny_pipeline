...
def write_phylip (concat_dict, outfile):
	...
	with open(outfile, "w") as OUTFILE:
		...
		len_align = len(list(concat_dict.values())[0]) # gets value of first element in dictionary -> this OK since all seqs have same length
...
def walk_alignment (indict):
	...
	n_columns = len( list(indict.values())[0] )
...
def count_pairwise_distance_all_and_fourfold_degen ( raw_dict, fourfold_degen_dict ):
	...
	for pair in all_pairs:
		...
		diff = 0
		total = 0
		print (len( list(raw_dict.values())[0] ))
		for idx in range(len( list(raw_dict.values())[0] )):
			p = raw_dict[tax1][idx]
			q = raw_dict[tax2][idx]
			if p != "-" and q != "-":
				total += 1
		...
		diff = 0
		total = 0
		for idx in range(len( list(fourfold_degen_dict.values())[0] )):
...
in_alignment = read_wrapped_or_unwrapped_fasta(infilename)
#print in_alignment
